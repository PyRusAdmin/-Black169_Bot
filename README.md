...
1. Отправьте `/start` боту (с аккаунта OWNER_ID или добавленного администратора)
...
3. Используйте inline-кнопки:
    ...
    - 👥 Управление администраторами — добавление/удаление (только владелец)
...
| Роль | Описание | Права |
|------|----------|-------|
| **Владелец** | Задаётся в `.env` (OWNER_ID) | Все права + управление администраторами |
| **Администратор** | Добавляется владельцем через бота | Доступ к админ-панели |
| **Пользователь** | Все остальные | Только пользовательские функции |

Владелец может:
- Добавлять администраторов (по Telegram ID)
- Удалять администраторов
- Просматривать список администраторов

Владельцы из `.env` автоматически синхронизируются с БД при каждом запуске бота.
